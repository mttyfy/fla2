...
__global__ void __launch_bounds__ (WARP_SIZE * mma_tile_q * mma_tile_k)
    flash_attn_mma_stages_split_q_shared_kv_kernel(half * Q, half * K, half * V, half * O,
        ...
        fill_3D_regs<uint32_t>(RD,0);
        {
            ...
            ///// Q_smem[br][]
            #pragma unroll
            for (int i = 0 ;  i < (head_dim * br) / (numthreads * 8) ; i+=1 ) {
                /// "cp.async.cg.shared.global.L2::128B [%0], [%1], %2;\n" ::"r"(dst),       
                /// "l"(src), "n"(bytes))
                int tid_data_id = tid * 8 + i * numthreads * 8;
                int Q_smem_br_new = tid_data_id / head_dim;
                int Q_smem_k_new  = tid_data_id % head_dim;
                uint32_t Q_smem_addr =
                Q_smem_tile_start_ptr +
                (
                (Q_smem_k_new/ mma_k  * br * (mma_k + padq ) +
                ...
                + Q_smem_k_new ];     
                CP_ASYNC_CG(Q_smem_addr , Q_gmem_addr , 16);
            }
        ...
        for (int i = 0 ; i < seqlen / bc ; i += 1) {
            if (canprefetchKVg2s) {
                if (i == 1) {                    
                    ...
                    for (int j = 0 ; j <(bc * head_dim) / numthreads ; j+=8) {
                        ...
                        swizzle<mma_k>(K_smem_bc,((K_smem_k+j)%mma_k))
                        ) * sizeof(half);
                ...
                if (i == 0) {
                    ...
                    for (int k = 0 ; k < head_dim / mma_k ; k ++) {
                        ...
                        for (int j = 0 ; j < mma_warp_q ; j++) {
                            ...
                            + swizzle<mma_k>(lane_smem_Q_br,lane_smem_Q_k)) 
                            * sizeof(half);
            ...
            for (int k = 0 ; k < head_dim/mma_k;k++) {
                ...
                for (int j = 0 ; j < mma_warp_k; j++) {
                    ...
                    /// only 16 threads join in the 16 *  8 data 
                    /// 16 * 8 -> 8 * 16 mma requires
...
void launch_flash_attn_mma_stages_split_q_shared_kv(half * Q,
                                                    half * K,
                                                    half * V,
                                                    half * O,
                                                    const int QKV_batch,
...
template void launch_flash_attn_mma_stages_split_q_shared_kv<32, 2>(half*, half*, half*, half*, int, int, int, cudaStream_t);
template void launch_flash_attn_mma_stages_split_q_shared_kv<64, 2>(half*, half*, half*, half*, int, int, int, cudaStream_t);
template void launch_flash_attn_mma_stages_split_q_shared_kv<96, 2>(half*, half*, half*, half*, int, int, int, cudaStream_t);
template void launch_flash_attn_mma_stages_split_q_shared_kv<128,2>(half*, half*, half*, half*, int, int, int, cudaStream_t);
template void launch_flash_attn_mma_stages_split_q_shared_kv<160,2>(half*, half*, half*, half*, int, int, int, cudaStream_t);
template void launch_flash_attn_mma_stages_split_q_shared_kv<192,2>(half*, half*, half*, half*, int, int, int, cudaStream_t);
template void launch_flash_attn_mma_stages_split_q_shared_kv<256,2>(half*, half*, half*, half*, int, int, int, cudaStream_t);

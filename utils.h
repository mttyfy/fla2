...
static __device__ __forceinline__ int swizzle_permuter_j(int i, int j) {
...
template<const int mma_k = 16>
static __device__ __forceinline__ int swizzle(int i, int j) {
  return swizzle_permuter_j<mma_k,8>(i,j);
}


template<int kMmaAtomK>
static __device__ __forceinline__
int swizzle_with_block(int block_id, int row, int inner_j) {
    static_assert(kMmaAtomK == 16);

    int vec = inner_j >> 3;
    int off = inner_j & 7;

    int row_swizzle   = (row >> 2) & 1;
    int block_swizzle = (block_id >> 1) & 1;

    int phys_vec = vec ^ row_swizzle ^ block_swizzle;

    return (phys_vec << 3) | off;
}
#include <vector>
